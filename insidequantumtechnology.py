...
def iqt_scrape_sitemap(list_sitemap, ignore_cache, driver, debug=False):
    """
    ...
    for sitemap_url in list_sitemap:
        try:
            ...
            for entry in soup.find_all('url'):
                ...
                # Extract URL
                article['url'] = entry.find('loc').text[8:-3]
                
                # Extract last modified date
                article['date'] = entry.find('lastmod').text[8:-3]
...
def iqt_generate_article_html(url, articles_dict, ignore_cache, driver, debug=False):
    """
    ...
    post_content = soup.find('article', class_='single_post_article')
    ...
    if not post_content:
        ...
        metadata = soup.find('script', class_='aioseo-schema')
...
def insidequantumtechnology(model, articles_dict, ignore_cache=False, debug=False):
    ...
    try:
        ...
        # https://www.insidequantumtechnology.com/sitemap.xml
        list_sitemap = []
        
        list_sitemap.append('https://www.insidequantumtechnology.com/news-sitemap.xml')
        
        for k in range(2, 11):
            list_sitemap.append('https://www.insidequantumtechnology.com/news-sitemap' + str(k) + '.xml')      
        
        url_list = iqt_scrape_sitemap(list_sitemap, ignore_cache, driver, debug)
        
        number_urls = len(url_list)
        
        logger.info(f"Number of urls identified: {number_urls}")
        
        #url_list =[{'url':'https://www.insidequantumtechnology.com/news-archive/iqt-research-report-estimates-revenues-from-quantum-machine-learning-and-quantum-deep-learning-at-1-1-billion-in-2030-2/'}]
